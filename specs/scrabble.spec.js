...
describe('Player', () => {
  ...
  describe('hasWon', () => {
    test('returns false when score < 100', () => {
      const testPlayer = new Scrabble.Player('test player');
       expect(testPlayer.totalScore()).toBe(0);
       expect(testPlayer.hasWon()).toBe(false);

       testPlayer.play('dog');
       expect(testPlayer.totalScore()).toBe(5);
       expect(testPlayer.hasWon()).toBe(false);

       testPlayer.play('pig');
       expect(testPlayer.totalScore()).toBe(11);
       expect(testPlayer.hasWon()).toBe(false);

       testPlayer.play('cat');
       expect(testPlayer.totalScore()).toBe(16);
       expect(testPlayer.hasWon()).toBe(false);
    ...
    test('returns true when score == 100', () => {
      const testPlayer = new Scrabble.Player('test player');
       testPlayer.play('zzzzkfa');
       expect(testPlayer.totalScore()).toBe(100);
    ...
    test('returns true when score > 100', () => {
      const testPlayer = new Scrabble.Player('test player');
      testPlayer.play('zzzzzzz');
      expect(testPlayer.totalScore()).toBe(120);
      expect(testPlayer.hasWon()).toBe(true);
  ...
  describe('highestScoringWord', () => {
    ...
    test('returns the highest scoring word played', () => {
      ...
      testPlayer.play('pig');
      testPlayer.play('dog');
      expect(testPlayer.highestScoringWord()).toBe('pig');

    });

    test('throws an error if no words have been played', () => {
      const testPlayer = new Scrabble.Player('test player');
      expect(() => { testPlayer.highestScoringWord(); }).toThrow();
    });
...
describe('highestWordScore', () => {
  test('returns the score of the highest scoring word played', () => {
    ...
    testPlayer.play('pig');
    testPlayer.play('dog');
    expect(testPlayer.highestWordScore()).toBe(6);
  ...
  test('throws an error if no words have been played', () => {
    const testPlayer = new Scrabble.Player('test player');
    expect(() => { testPlayer.highestWordScore(); }).toThrow();
