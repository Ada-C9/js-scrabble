...
describe('Player', () => {
  ...
  describe('hasWon', () => {
    test('returns false when score < 100', () => {
      const player = new Scrabble.Player('test player');
      expect(player.play('kk')).toBeTruthy(); // +10 pts
      expect(player.plays.length).toBe(1);
    ...
    test('returns true when score == 100', () => {
      const player = new Scrabble.Player('test player');
      expect(player.play('zzzkkkk')).toBeTruthy(); // +100 pts
      expect(player.plays.length).toBe(1);
  ...
  describe('highestScoringWord', () => {
    ...
    test('returns the highest scoring word played', () => {
      const player = new Scrabble.Player('test player');
      expect(player.play('donkey')).toBeTruthy();
      expect(player.play('apple')).toBeTruthy();
      expect(player.play('maybe')).toBeTruthy();
      expect(player.highestScoringWord()).toBe('donkey')
    });
  ...
  describe('highestWordScore', () => {
    test('returns the score of the highest scoring word played', () => {
      ...
      expect(player.play('donkey')).toBeTruthy();
      expect(player.play('apple')).toBeTruthy();
      expect(player.highestWordScore()).toBe(14)
    });
